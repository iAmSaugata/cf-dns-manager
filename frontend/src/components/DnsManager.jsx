...
export default function DnsManager({ zone, onSignOut, onChangeZone }){
  ...
  useEffect(()=>{
    (async ()=>{
      setLoading(true)
      try{ const d = await api.listRecords(zone.id); setRows(d.result||[]) }finally{ setLoading(false) }
    })()
  }, [zone?.id])
  ...
  const save = async ()=>{
    ...
    try{
      ...
      const d = await api.listRecords(zone.id); setRows(d.result||[]); setShowModal(false); setEditing(null)
  ...
  const delSelected = async ()=>{
    ...
    setConfirmDel(false)
    for (const id of ids){ try{ await api.deleteRecord(zone.id, id) }catch(e){ console.error('Delete failed', id, e) } }
      ...
      <div className="container">
        ...
        <div className="row nowrap" style={{marginBottom:12}}>
          <select value={type} onChange={e=>setType(e.target.value)} style={{maxWidth:160}}>
            <option>All</option>{ALLOWED_TYPES.map(t => <option key={t}>{t}</option>)}
          </select>
            ...
            {filtered.map(r=>{
              ...
              return (
                <tr key={r.id}>
                  ...
                  <td>{truncate(r.content)}{r.type==='MX' && (r.priority!==undefined && r.priority!==null) ? <span className="badge-priority">{r.priority}</span> : null}</td>
                  <td>{r.ttl === 1 ? 'Auto' : r.ttl}</td>
                  <td>{proxyCell(r)}</td>
                  <td className="actions">
                    <button className="btn" onClick={()=>!disabled && openEdit(r)} disabled={disabled}>Edit</button>
                    <button className="btn red" onClick={()=>{ if(!disabled){ setSelected({[r.id]:true}); setConfirmDel(true);} }} disabled={disabled}>Delete</button>
                  </td>
                </tr>
              )
            })}
        ...
        {showModal && (
          <Modal onClose={()=>setShowModal(false)}>
            ...
            <div className="form-grid">
              <div className="form-field">
                <label style={{display:'flex', alignItems:'center', gap:8}}>Proxy
                  {['A','AAAA','CNAME'].includes(editing.type) ? (
                    ...
                    <span className="kv" style={{marginLeft:8}}>DNS only</span>
                  )}
                </label>
              </div>
              <div className="form-field">
                ...
                {editing.type==='MX' ? (
                  <input className="input" type="number" min="0" value={(editing.priority ?? 0)} onChange={e=>setEditing({...editing, priority:e.target.value})} />
                ) : (
                  <div className="input" style={{color:'#9ca3af', background:'#f8fafc'}}>N/A</div>
                )}
              </div>
            </div>
              ...
              <div>
                ...
                <ul>
                  {Object.entries(selected).filter(([id, v])=>v).map(([id])=>{
                    ...
                    return <li key={id}><b>{r.type}</b> — {r.name} — {r.content}</li>
                  })}
                </ul>
                <div className="modal-actions">
