...
// Simple API helper that passes password as header and disables caching
const api = async (p,o={})=>{
...
function ZonePicker({onPick}){
  ...
  useEffect(()=>{(async()=>{
    try{
      const d=await api(`/zones?t=${Date.now()}`)
      const zs=d.result||[]; setZones(zs)
  ...
  return (
    <div className="center" style={{alignItems:'flex-start'}}>
      <div className="wrap" style={{padding:0}}>
        <div className="header" style={{padding:'18px'}}>
          ...
          {(zones||[]).map(z=>(
            <div key={z.id} className="zone-card">
              <div className="zone-header">
                ...
                <div className="badge">{(z.plan && z.plan.name) ? z.plan.name : 'Free'}</div>
              </div>
              <div className="zone-open">
...
function Records({zone,onBack}){
  ...
  const load=async()=>{
    ...
    try{
      const d=await api(`/zone/${zone.id}/dns_records?per_page=200&t=${Date.now()}`)
      setRecs(d.result||[])
    }catch(e){ setErr(e.message) } finally { setLoading(false) }
  }
  ...
  return (
    <div className="wrap">
      ...
      <div className="card">
        {/* Toolbar line 1: filters + search + clear (left) and delete selected (right) */}
        ...
        {!loading && !err && (
          <>
            <div className="grid th">
              <div><input type="checkbox" checked={allSelected} onChange={e=>toggleAll(e.target.checked)} /></div>
              <div>Type</div><div>Name</div><div>Content</div><div>TTL</div><div>Proxy</div><div>Actions</div>
            </div>
            {filtered.map(r=> (
              <Row
                key={r.id}
...
function Login({onDone}){
  ...
  const go=async()=>{
    ...
    try{
      ...
      const r=await fetch('/api/health',{headers:{'x-app-password':p},credentials:'same-origin'})
  ...
  return (
    <div className="center">
      <div className="card" style={{width:'100%',maxWidth:520,padding:'22px'}}>
        ...
        <div className="login-actions" style={{marginTop:12}}>
          ...
          <button className="btn" disabled={busy||!p} onClick={go}>{busy?'Checking…':'Login'}</button>
        </div>
        {err && <div className="muted" style={{marginTop:8}}>Error: {err}</div>}
      </div>
    </div>
  )
}
