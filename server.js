...
async function cfFetch(path, options={}){ const url=`${CF_BASE}${path}`; console.log('[CF]', options.method||'GET', url);
  ...
  let data={}; try{data=await res.json();}catch{} if(!res.ok||data?.success===false){ const err=new Error(data?.errors?.[0]?.message||`Cloudflare API error (${res.status})`); err.status=res.status; err.cf=data; throw err;} return data; }
app.get('/api/zones', async (req,res,next)=>{ try{ res.json(await cfFetch('/zones?per_page=200&page=1')); }catch(e){ next(e);} });
app.get('/api/zone/:zoneId/dns_records', async (req,res,next)=>{ try{ const {zoneId}=req.params; const {page=1,per_page=200,type='',name='',content=''}=req.query; const q=new URLSearchParams({page,per_page}); if(type) q.set('type',type); if(name) q.set('name',name); if(content) q.set('content',content); res.json(await cfFetch(`/zones/${zoneId}/dns_records?${q.toString()}`)); }catch(e){ next(e);} });
...
const distPath = path.join(__dirname,'frontend','dist'); app.use(express.static(distPath)); app.get('*',(_,res)=>res.sendFile(path.join(distPath,'index.html')));
